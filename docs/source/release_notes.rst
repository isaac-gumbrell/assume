...
**New Features:**
  ...
  - **Runnable staggered superset scenarios**: staggered (paired-scenario) training now writes a self-contained ``_superset`` folder per scenario (union unit CSVs, copies of all other inputs, and a ``foreign_units.json`` manifest). The folder can be run standalone in non-learning mode with the learned policies; the loader reads the manifest and forces foreign generators to zero output (``availability = 0`` / ``demand = 0``). A new helper ``run_staggered_evaluation`` runs both superset scenarios with the shared trained policy.
