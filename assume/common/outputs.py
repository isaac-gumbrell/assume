...
class WriteOutput(Role):
    ...
    def convert_flows(self, data: pd.DataFrame | dict[tuple[datetime, str], float]):
        """
        Convert the flows of the grid results into a dataframe.

        Args:
            data: Either a long-format DataFrame with index ``datetime`` and columns
                  ``[line, flow, congestion_pct]`` (produced by the clearing algorithms
                  when ``log_flows=True``), or a legacy ``{(datetime, line): flow}``
                  dict (preserved for backwards compatibility).
        """
        # if data is dataframe — primary path from nodal/complex clearing
        ...
        # if data is list
            ...
            # Legacy format: {(datetime, line): flow_value}
